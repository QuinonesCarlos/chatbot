...
const STORE_SESIONES = "SELECT `id_cliente`, `session`, `celular`, `alias` FROM " + _STORE_ + ".`whatsapp` WHERE eliminada = 0;";

const Whatsapp = require("./whatsapp.js");
data.consultarCallback(STORE_SESIONES, [], function (sesiones) {
    for (var i = 0; i < sesiones.length; i++) {
        _MAP_MESSAGE.set(`${sesiones[i]['id_cliente']}-${sesiones[i]['celular']}`, new Whatsapp(sesiones[i]['id_cliente'], sesiones[i]['celular'], sesiones[i]['alias'], JSON.parse(sesiones[i]['session'])));
    }
});

const STORE_ONLINE =
    "UPDATE " + _STORE_ + ".`cliente_session` SET id = ? WHERE `id_cliente` = ? AND `imei` = ? LIMIT 1;";
...
EMIT_ = require('socket.io')(http).on('connection', function (socket) {
    if (!socket.handshake.query.auth || !socket.handshake.query.id)
        ...
        data.consultar(STORE_ONLINE, [socket.id, socket.handshake.query.id, socket.handshake.query.auth]);
